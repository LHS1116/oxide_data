...
class News(object):
    ...
    time: int
    type: str
    content: str# enum "msg" | "gov"; # 可能不会用到这些type，取决于数据能否整理出
...
def format_reports(_line):
    ...
    if title is not None:
        gov_news_yqtb = News()
        dt = datetime.strptime(_line.span.text, "%Y-%m-%d")
        gov_news_yqtb.time = get_timestamp(dt)
        gov_news_yqtb.id = 0
        if title[0] != '截':
            title = f'{dt.date()} {title}'
        gov_news_yqtb.title = title[:200]
        gov_news_yqtb.link = link
        gov_news_yqtb.type = 'gov'
        gov_news_yqtb.id = f"news_gov_{fun(link)}"
        gov_news_yqtb.content = ""
...
def get_isaa_news():
    ...
    for news in newses:
        ...
        ns.content = news.get('summary', "")[:900]
        ns.id = f"news_{sha_256(ns.link.encode())}"
        ns.type = 'msg'
